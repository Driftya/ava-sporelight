...
Describe "New-CmsContentPackage" {
    ...
    It "builds a complete checksummed package for the collection and 35 chapters" {
        ...
        (Get-Content -LiteralPath (Join-Path $extractPath "pages/collection.md") -Raw) | Should Not Match "01-cover-image"
    ...
    It "promotes image 01 to the cover and keeps later ordered images inline" {
        ...
        [System.IO.File]::WriteAllText((Join-Path $fixtureRoot "manuscript/000-front-matter.md"), "---`nid: ava-sporelight-front-matter`ntitle: Ordered Images`ntype: front-matter`n---`n# Ordered Images", $utf8)
